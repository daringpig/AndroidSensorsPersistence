...
public abstract class RecordRepositoryIntegrationTest<T extends SensorRecord> {

    protected RecordRepository<T> recordRepository;

    public void setUp() throws Exception {
        if (recordRepository == null) {
            throw new Error("You must inject a recordRepository on setUp method");
        }
    }
    ...
    public void checkThatStoresAndRetrievesData() throws Exception {
        ...
        recordRepository.createAll(sensorRecords).blockingGet();
        List<T> retrievedSensorRecords =
                recordRepository.findAll().toList().blockingGet();
    ...
    public void checkThatStoresAndRetrievesDataWithForeignKey() throws Exception {
        ...
        long foreignKey = 5L;
        recordRepository.createAll(sensorRecords, foreignKey).blockingGet();
        List<T> retrievedSensorRecords =
                recordRepository.findAllBy(foreignKey).toList().blockingGet();
